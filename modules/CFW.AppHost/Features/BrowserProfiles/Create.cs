...
public class BrowserProfilesCreate : IApiOperationHandler<CreateProfileRequest, CreateProfileResponse>
{
    public async Task<IResult<CreateProfileResponse>> Handle(CreateProfileRequest request, CancellationToken cancellationToken)
    {
        var result = new CreateProfileResponse();
        var gitHubApi = RestService.For<IProfileGateway>("http://127.0.0.1:53200/api/v2");
        result = await gitHubApi.CreateProfileAsync(request);
